...
<div class="container" >
  <div class="span3">
		    <form id="CommodityTypeForm" action="${ctx}/Commodity/newadd.do" method="post" style="float: left;">
				<div class="row-fluid">
				...
				<div class="row-fluid">
					<button style="margin-top:-23px;" class="btn btn-info btn-small" onclick="CommodityTypeForm()" type="button" ><li class="icon-plus icon-white"></li>新增</button>
    ...
    <br>
    <div class="dgvone" style="width:260px;height:403px;overflow:scroll;">
       <table class="table table-condensed table-bordered table-striped" id="tableid" style="width: 260px;height: 20px;">
	      <thead class="theadone">
	        <tr>
	          <th rowspan="2">选择</th>
	          <th rowspan="2">商品类别</th>
 ...
 */
      ...
      $(function(){
          typeC.on("blur",function() {
              if (typeC.val() != null && typeC.val() != "") {
                  jQuery.ajax({
                      type: "POST",//请求类型
                      url: getRootPath() + "/Commodity/validationCommodityType.do",//请求的url
                      data: {CommodityType: typeC.val()},//请求参数
                      dataType: "json",//ajax接口（请求url）返回的数据类型
                      success: function (data) {//data：返回数据（json对象）
                          if (data.CommodityTypeCode == "exist") {//账号已存在，错误提示
                              alert("商品类型已经存在");
                              flag2=false;
                          }else{
                              flag2=true;
      ...
      function CommodityTypeForm(){
          if(flag2){
              $("#CommodityTypeForm").submit();
          }
      }
